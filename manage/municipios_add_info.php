...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_municipios($selected_id, $_POST['description'], $_POST['codigo']);
			$note = _('Selected Municipios has been updated');
    	} 
    	else 
    	{
    		add_municipios($_POST['description']);
...
$result = get_municipioss(check_value('show_inactive'));
...
while ($myrow = db_fetch($result)) 
{
	...
	inactive_control_cell($myrow["municipios_code"], $myrow["inactive"], 'addfields_municipios', 'municipios_code');

 	edit_button_cell("Edit".$myrow["municipios_code"], _("Edit"));
 	delete_button_cell("Delete".$myrow["municipios_code"], _("Delete"));
	end_row();
}
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_municipios($selected_id);
